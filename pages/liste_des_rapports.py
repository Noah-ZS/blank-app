...
st.markdown(
    ...
    /* Selected folder / ancestor-of-selected — matches the original
       .tree-active / .tree-parent-active rules exactly: color +
       weight only, no background highlight. */
...
if st.session_state.lr_active_tab in REPORT_TABS:
    ...
    def _render_folder_tree(nodes, depth=0):
        for node in nodes:
            ...
            if is_star:
                prefix = "\u00A0 \u00A0 "
                glyph = ":material/star:" if is_active else ":material/star_border:"
            elif has_children:
                prefix = ":material/expand_more: " if is_expanded else ":material/chevron_right: "
                glyph = ":material/folder_open:" if is_expanded else ":material/folder:"
            else:
                prefix = "\u00A0 \u00A0 "
                glyph = ":material/folder:"
